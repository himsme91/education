...
<div class="<?php print $classes; ?>">
  ...
  <?php else:?>
    <?php
    	$pos = strpos($view->args[2], 'PIf');
		if ($pos !== false){
			$rat_sys = taxonomy_get_term($view->args[0]);
			$rat_sys_ver = taxonomy_get_term($view->args[1]);
			$path = 'credits/'.str_replace('&', '%26', usgbc_dashencode($rat_sys->name)).'/'.str_replace('&', '%26', usgbc_dashencode($rat_sys_ver->name)).'/minimum-program-requirements';
