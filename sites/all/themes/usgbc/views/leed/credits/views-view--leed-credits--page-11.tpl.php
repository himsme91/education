<?php 
$rating_system_name = $view->args[0];
$version_name = $view->args[1];
$credits_list = $view->result;
...
switch($rating_system_name){
	...
	break;
	case 'data-centers---new-construction':
	$rating_system_description = 'LEED for New Construction for Data Centers';
	break;
	case 'hospitality---new-construction':
	$rating_system_description = 'LEED for New Construction in Hospitality';
	break;
	case 'warehouse-and-distribution-centers---new-construction':
	$rating_system_description = 'LEED for New Construction in Warehouses and Distribution Centers';
	break;
	case 'hospitality---commercial-interiors':
	$rating_system_description = 'LEED for Commercial Interiors in Hospitality';
	break;
	case 'schools---new-construction':
	$rating_system_description = 'LEED for New Construction in Schools';
	break;
	case 'mid-rise':
	$rating_system_description = 'LEED for Homes - Mid Rise';
	break;
...
foreach($credits_list as $credit){
	$shortid  = $credit->node_data_field_credit_short_id_field_credit_short_id_value;
	$name     = $credit->node_title;
	$nodeid   = $credit->nid;	
	$possible = $credit->node_data_field_credit_short_id_field_credit_points_possible_upp_value;
	$rpc      = $credit->node_data_field_credit_short_id_field_credit_rpc_flag_value == '1';
	$required = $credit->node_data_field_credit_short_id_field_credit_required_flag_value == "Required";
	$catid	  = $credit->node_data_field_credit_category_field_credit_category_value;
	$catname  = taxonomy_get_term($catid);
	$catname = $catname->name;
	
	$shortcat = strtolower(substr($shortid,0,2));
	$shortcat2 = strtolower(substr($shortid,0,3));	
	if($shortcat == 'lt') $shortcat = 'sl';
	...
	if($shortcat != 'pc' && $shortcat != 'mp' && $catname != 'Innovation catalog' && $shortcat2 != 'int'){
		...
		if(inStr($version_name, 'v4')){
			if($rating_system_name == 'new-construction' 
				|| $rating_system_name == 'retail---new-construction' 
				|| $rating_system_name == 'data-centers---new-construction'
				|| $rating_system_name == 'hospitality---new-construction'
			|| $rating_system_name == 'warehouse-and-distribution-centers---new-construction') {
				$categories['sl']['possible'] = 16;
				$total_possible               = 110;
			}
			if($rating_system_name == 'commercial-interiors' 
				|| $rating_system_name == 'retail---commercial-interiors' 
			|| $rating_system_name == 'hospitality---commercial-interiors') {
				$categories['sl']['possible'] = 18;
				$total_possible               = 110;
			}
			if($rating_system_name == 'core-and-shell'){ 
				$categories['sl']['possible'] = 20;
				$total_possible                = 110;
			}					
			if($rating_system_name == 'core-and-shell'){
				$categories['sl']['possible']  = 20;
			...
			if($rating_system_name == 'schools---new-construction'){
				$categories['sl']['possible']  = 15;
				$total_possible                 = 110;
			}
			if($rating_system_name == 'healthcare'){
				$categories['sl']['possible']  = 9;
				$total_possible                 = 110;
			}
			if($rating_system_name == 'homes'){
				$total_possible                = 110;
				$categories['sl']['possible']  = 15;
				$categories['we']['possible']  = 12;				
				$categories['ea']['possible']  = 38;								
			}
			if($rating_system_name == 'mid-rise'){
				$total_possible                = 110;
				$categories['sl']['possible']  = 15;
				$categories['we']['possible']  = 12;				
			}
...
/*		font-family: "gotham-narrow";*/
	...
	.scorecard .sl table.category-head tr td{
		border-bottom: 3px solid #314556;
	}	
	.scorecard .ss table.category-head tr td{
 		color: #A2CB00;
	}
	...
	#wrapper .scorecard .header td div,
	#wrapper .project-badge,
	#wrapper .pdf-view,			
	#wrapper .scorecard .header td h1{
		display:none;
	}
...
<div class="scorecard">
	<table style="width:100%" class="header" cellpadding="0" cellspacing="0">
			<tbody><tr>
				<td valign="bottom" style="width:30px">
					<img src="https://new.usgbc.org/reviewreport/images/leed-logo.png" height="28px" width="28px" alt="GBCI logo" class="logo">
				</td>
				<td class="border">
					<h2 style="float:left;color:black;font-weight:700"><?php print $rating_system_description;?></h2>
				</td>
			</tr>
		</tbody>
	</table>	
	<?php
		...
		if($rows_categories['ip'])		render_category($rows_categories['ip'], null, 0);		
		...
		if($mr_break > 0 && $rows_categories['mr']) render_category($rows_categories['mr'], "second", $mr_break);
		if($mr_break == -1 && $rows_categories['mr']) render_category($rows_categories['mr'], null, 0);		
		...
		print "<td class='points-label'>".$total_possible."</td>";						
		print "</tr>";
		print "</table>";
...
//		if($category['awarded'] == 0) $category['awarded'] = '';
		...
		print "<div style='position:relative;float:left;margin-top:17px'>";
		if($category['id']!='ip'){
			print "<img height=30 width=30 src='".$image_path."'/>";
		}
		...
		if($half == 'second')
			...
			print "<td class='points-label'>Possible: ".$category['possible']."</td>";
		print "</tr>";
		print "</table>";	
